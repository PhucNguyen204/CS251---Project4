...
const exchange_name = 'Chill Exchange';             // TODO: fill in the name of your exchange

const token_name = 'Puckuncute';                // TODO: replace with name of your token
const token_symbol = 'PUC';              // TODO: replace with symbol for your token
...
/*** INIT ***/
var pool_State; // lưu trạng thái của pool
...
async function addLiquidity(amountEth, maxSlippagePct) {
    ...
    var max_exchange_rate = Math.round((parseFloat(pool_state.token_eth_rate) * (1 + maxSlippagePct / 100)) * Math.pow(10, 18));
    var min_exchange_rate = Math.round((parseFloat(pool_state.token_eth_rate) * (1 - maxSlippagePct / 100)) * Math.pow(10, 18));
    let allowance = await token_contract.connect.allowance(defaultAccount, exchange_address);
    if (allowance < amountEth) {
      await token_contract.connect(provider.getSigner(defaultAccount)).approve(exchange_address, amountEth);
    }
    return await exchange_contract.connect(provider.getSigner(defaultAccount)).addLiquidity(amountEth, max_exchange_rate, min_exchange_rate, { value: ethers.utils.parseUnits(amountEth.toString(), "wei") });
...
async function removeLiquidity(amountEth, maxSlippagePct) {
    ...
    return await exchange_contract.connect(provider.getSigner(defaultAccount)).removeLiquidity(amountEth, max_exchange_rate, min_exchange_rate);
...
async function swapTokensForETH(amountToken, maxSlippagePct) {
    let pool_state = await getPoolState();
    var max_exchange_rate = Math.round((parseFloat(pool_state.token_eth_rate) * (1 + maxSlippagePct / 100)) * Math.pow(10, 18));
    await token_contract.connect(provider.getSigner(defaultAccount)).approve(defaultAccount, amountToken);
...
async function swapETHForTokens(amountEth, maxSlippagePct) {
    let pool_state = await getPoolState();
    var max_exchange_rate = Math.round((parseFloat(pool_state.token_eth_rate) * (1 + maxSlippagePct / 100)) * Math.pow(10, 18));
    return await exchange_contract.connect(provider.getSigner(defaultAccount)).swapETHForTokens(max_exchange_rate, { value: ethers.utils.parseUnits(amountEth.toString(), "wei") });
